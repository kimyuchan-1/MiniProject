...
- [ ] 1. Spring Boot 백엔드 프로젝트 설정
  - Spring Boot 3.x 프로젝트 생성 및 의존성 설정 (JPA, Security, Session JDBC, Lombok, MySQL)
  - MySQL 연결 설정 및 JPA 구성
  - Spring Security 및 세션 기반 인증 기본 설정
  - 기본 프로젝트 구조 생성
  - _요구사항: 5.5, 12.1_
...
- [ ] 5. 세션 기반 인증 및 보안 구현
  - [ ] 5.1 Spring Security 세션 설정 구현
    - SecurityConfig 클래스 작성 (세션 기반 인증)
    - 세션 관리 및 CORS 설정
    - 사용자 권한별 API 접근 제어
  ...
  - [ ] 5.3 인증 핸들러 및 API 구현
    - 로그인/로그아웃 성공/실패 핸들러 구현
    - AuthController 구현 (회원가입, 로그인 상태 확인)
    - 세션 기반 사용자 정보 반환 API
    - _요구사항: 12.3, 12.4, 12.5_

  - [ ]* 5.4 속성 테스트 - 세션 인증 상태 관리 작성
    - **속성 16: 세션 인증 상태 관리**
    - **검증: 요구사항 12.2, 12.4**
...
- [ ] 13. Next.js API 라우트 구현
  - [x] 13.1 백엔드 프록시 API 라우트 작성

    - 월별 데이터, KPI, 예측 분석 데이터 프록시
    - 환경변수를 통한 백엔드 URL 설정
    - 세션 쿠키 전달을 위한 credentials 설정
    - _요구사항: 1.4, 9.1, 10.1, 12.2_

  - [x] 13.2 인증 관련 API 라우트 구현

    - 세션 기반 로그인/로그아웃 API 프록시
...
- [ ] 14. KPI 중심 대시보드 UI 구현
  - [x] 14.1 KPI 대시보드 컴포넌트 구현

    - 신호등 설치율, 사고 감소율, 안전 지수 표시
    - 지역별 KPI 비교 및 트렌드 차트
    - _요구사항: 1.5, 4.1_

  - [x] 14.2 월별 트렌드 분석 UI 구현
