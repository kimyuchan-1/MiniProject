...
export interface SafetyWeights {
  ...
  crosswalkWidth: number;      // 횡단보도 폭 (넓을수록 안전)
...
export interface EnhancedCrosswalk {
  cw_uid: string;
  sido: string;
  sigungu: string;
  address: string;
  ...
  button?: boolean;             // 보행자작동신호기유무
  sound_signal?: boolean;       // 음향신호기설치유무
  bump?: boolean;               // 보도턱낮춤여부
  braille_block?: boolean;      // 점자블록유무
...
export function calculateRiskScore(accident: AccidentData, weights: RiskWeights = DEFAULT_RISK_WEIGHTS): number {
  ...
  // 0-100 스케일로 정규화 (임시로 최대값 100으로 설정)
  return Math.min(rawScore, 100);
}

export function calculateSafetyScore(crosswalk: EnhancedCrosswalk, weights: SafetyWeights = DEFAULT_SAFETY_WEIGHTS): number {
  let score = 0;
  
  if (crosswalk.hasSignal) score += weights.hasSignal;
  if (crosswalk.button) score += weights.hasButton;
  if (crosswalk.sound_signal) score += weights.hasSound;
  if (crosswalk.highland) score += weights.isHighland;
  if (crosswalk.bump) score += weights.hasBump;
  if (crosswalk.braille_block) score += weights.hasBraille;
  if (crosswalk.spotlight) score += weights.hasSpotlight;
  
  // 횡단보도 폭 가중치 (있는 경우)
  if (crosswalk.crosswalk_width) {
    score += crosswalk.crosswalk_width * weights.crosswalkWidth;
  }
